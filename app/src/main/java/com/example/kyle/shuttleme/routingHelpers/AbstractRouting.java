...
import com.example.kyle.shuttleme.routingHelpers.parsers.GoogleParser;
...
public abstract class AbstractRouting<T> extends AsyncTask<T, Void, Route> {
    ...
    protected void dispatchOnSuccess(PolylineOptions mOptions, Route route) {
        for (RoutingListener mListener : _aListeners) {
            ...
            mListener.onRoutSuccess(mOptions, route);
            mListener.onRSuccess(mOptions, route);
            mListener.onRouteS(mOptions, route);
            mListener.onRouSuccess(mOptions, route);
        }
    }
